...
export default function GlassIconsSection({ items, contentMapping }: GlassIconsSectionProps) {
  ...
  return (
    <div className="py-16">
      <h2 className="text-2xl font-bold text-foreground text-center mb-4">Mecânicas do Jogo</h2>

      <GlassIcons items={items} onSelect={(value: string) => setSelectedMechanic(value)} className="max-w-2xl" />
      ...
      <Dialog open={!!selectedMechanic} onOpenChange={handleOpenChange}>
        <DialogContent>
          ...
          {content && <video src={"/videos/" + content.video} controls autoPlay muted className="w-full rounded-lg" />}
          {content && <div className="text-sm text-foreground whitespace-pre-line">{content.details.trim()}</div>}
        </DialogContent>
      </Dialog>
    </div>
  );
